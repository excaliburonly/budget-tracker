...
import { getCategories, getTransactions, deleteTransaction } from "./actions";
import { getEmergencyFunds } from "../emergency-funds/actions";
import { getInvestments } from "../investments/actions";
import { getAccounts } from "../accounts/actions";
import { AddTransactionForm, AddCategoryForm, EditTransactionModal } from "./TransactionForms";
import { Transaction, Category, EmergencyFund, Investment, Account } from "@/types/database";
...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [cats, trans, funds, invs, accs] = await Promise.all([
        getCategories(),
        getTransactions(),
        getEmergencyFunds(),
        getInvestments(),
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-900 dark:text-white">
                          {t.type === 'transfer' ? (
                            <span className="text-indigo-600 dark:text-indigo-400 font-bold">Transfer</span>
                          ) : (
                            t.notes || '-'
                          )}
                        </span>
