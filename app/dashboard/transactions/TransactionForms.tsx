...
import {addCategory, addTransaction, deleteCategory, updateCategory, updateTransaction} from "./actions";
import {Account, Category, EmergencyFund, Investment, Transaction} from "@/types/database";

import {PencilSquareIcon, TrashIcon} from "@heroicons/react/24/outline";

export function AddTransactionForm({
                                       categories, emergencyFunds = [], investments = [], accounts = []
                                   }: {
    categories: Category[], emergencyFunds?: EmergencyFund[], investments?: Investment[], accounts?: Account[]
}) {
    ...
    return (<div
            className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm mb-8">
            ...
            <form action={addTransaction} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {type !== 'transfer' ? (<div className="flex flex-col gap-1.5">
                        ...
                        <label className="text-sm font-medium text-gray-700 dark:text-gray-300">To Account
                            (Destination)</label>
                        <select
                            name="to_account_id"
                ...
                <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Link to Emergency
                        Fund</label>
                    <select
                    ...
                    >
                        ...
                        {emergencyFunds.map((f) => (<option key={f.id} value={f.id}>
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    >
                        ...
                        {investments.map((i) => (<option key={i.id} value={i.id}>
...
export function EditTransactionModal({
    ...
    return (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div
                className="bg-white dark:bg-gray-900 p-8 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-2xl max-w-2xl w-full">
                ...
                <form action={async (formData) => {
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Link to
                            Investment</label>
                        <select
                            name="investment_id"
...
export function AddCategoryForm({categories}: { categories: Category[] }) {
    ...
    return (<div
            className="bg-gray-50 dark:bg-gray-800/50 p-6 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700 mb-8">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-md font-semibold text-gray-900 dark:text-white">Categories</h3>
                <button onClick={() => setShowForm(false)}
                        className="text-xs text-gray-500 hover:text-gray-700 font-medium">Close
                </button>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
              ...
              {category.type}
                        ...
                        <div
                            className="flex items-center justify-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 w-full">
                            <button
            ...
            {editingCategory && (<EditCategoryModal
                    ...
                    onCloseAction={() => setEditingCategory(null)}
                />)}
        </div>);
}
...
export function EditCategoryModal({category, onCloseAction}: { category: Category, onCloseAction: () => void }) {
    return (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div
                className="bg-white dark:bg-gray-900 p-8 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-2xl max-w-lg w-full">
