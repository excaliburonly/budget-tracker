...
export default async function DashboardPage() {
    ...
    const expenseByCategoryData = Object.values(transactions
        ...
        .reduce((acc: Record<string, { name: string; value: number; color: string }>, t: Transaction) => {
            ...
            if (!acc[catId]) {
                acc[catId] = {
                    name: catName, value: 0, color: t.categories?.color || '#ef4444'
                };
            }
    ...
    const incomeByCategoryData = Object.values(transactions
        ...
        .reduce((acc: Record<string, { name: string; value: number; color: string }>, t: Transaction) => {
            ...
            if (!acc[catId]) {
                acc[catId] = {
                    name: catName, value: 0, color: t.categories?.color || '#10b981'
                };
            }
            acc[catId].value += Number(t.amount);
            return acc;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            <div
                ...
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm font-medium text-text-muted uppercase tracking-wider">Daily Budget</span>
                </div>
                <div
                    className={`text-2xl md:text-3xl font-bold mt-2 ${moneyPerDay >= 0 ? 'text-primary' : 'text-red-600'}`}>
                    {formatCurrency(moneyPerDay, currency)}
                </div>
                <span className="text-xs text-text-muted mt-2 inline-block">Remaining: {daysLeft} day{daysLeft === 1 ? '' : 's'}</span>
            </div>
            ...
            <div
                ...
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span
                        className="text-sm font-medium text-text-muted uppercase tracking-wider">Monthly Expenses</span>
                </div>
            ...
            <section className="lg:col-span-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-6">
                            <ChartPieIcon className="w-6 h-6 text-foreground"/>
                            <h3 className="text-xl font-bold text-foreground">Income by Category</h3>
                        </div>
                        <div className="bg-surface p-6 rounded-2xl border border-surface-border shadow-sm">
