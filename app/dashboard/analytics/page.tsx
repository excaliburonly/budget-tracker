...
export default async function AnalyticsPage(props: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const [transactions, categories, allInvestments] = await Promise.all([
    getTransactions(),
    getCategories(),
    getInvestments()
  ]);
  ...
  const dailyData: Record<string, string | number>[] = Array.from({ length: 31 }, (_, i) => {
  ...
  let mfPerformance: PerformancePoint[] = [];
  let stockPerformance: PerformancePoint[] = [];
  const individualPerformances: Record<string, PerformancePoint[]> = {};

  if (activeTab === "investments") {
    [mfPerformance, stockPerformance] = await Promise.all([
    ...
    individualResults.forEach(res => {
      individualPerformances[res.id] = res.data;
    });
  }
